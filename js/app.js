...
function getUserRole() {
    return getCurrentUser().role?.trim() || '';
}


// ==================== الصلاحيات ====================
function isViewOnly() {
    return getUserRole() === 'عرض فقط';
}
...
async function loadPage(page) {
    ...
    switch (page) {
        ...
        case 'addStation':
            ...
            if (!canAdd()) {
                container.innerHTML = `<div class="alert alert-danger">ليس لديك صلاحية</div>`;
                return;
            }
            ...
            container.innerHTML = `
                <div class="card p-3">
                    ...
                    <input id="stationName" class="form-control mb-2" placeholder="اسم المحطة">

                    <button class="btn btn-success" onclick="saveStation()">حفظ</button>
                </div>
            `;
            break;
        ...
        case 'listStations':
            ...
            data.forEach(st => {
                ...
                if (canEdit()) {
                    actions += `<button class="btn btn-warning btn-sm" onclick="editStation(${st.id})">✏️</button> `;
                }

                if (canDelete()) {
                    actions += `<button class="btn btn-danger btn-sm" onclick="deleteStation(${st.id})">🗑️</button>`;
                }
        ...
        default:
            container.innerHTML = `<div class="alert alert-warning">الصفحة غير موجودة</div>`;
    }
}
...
// ==================== دوال التقارير الشهرية والمتابعة ====================
        ...
        if (reports.length === 0) {
            ...
            reports.forEach(r => {
                html += `运转
                    ...
                    <td>${r.totalElecCost || 0} ج</td>
                    <td>${r.totalDieselBalance || 0} لتر</td>
                    <td>
...
async function loadReportDaily(container) {
    ...
    window.printDailyReport = (id) => {
        ...
        if (report) {
            ...
            <table><thead>运转<th>الأصل</th><th>الحالة</th><th>ملاحظات</th><th>سبب التوقف</th> </thead><tbody>
...
async function loadReportFaults(container) {
    ...
    window.showFaultsReport = () => {
        ...
        let html = '<div class="table-responsive"><table class="table"><thead>运转<th>المحطة</th><th>الأصل</th><th>النوع</th><th>التاريخ</th><th>الوصف</th><th>الإجراء</th><th>الحالة</th> </thead><tbody>';
        filtered.forEach(f => {
            const station = stations.find(s => s.id == f.stationId);
            html += `<tr><td>${station?.name || f.stationName || '-'}</td><td>${f.assetName || '-'}</td><td>${f.type}</td><td>${f.date}</td><td>${f.description}</td><td>${f.actions || '-'}</td><td>${f.status === 'open' ? 'مفتوح' : 'مغلق'}</td></tr>`;
        });
        html += '</tbody>  </div>';
...
async function loadReportStation(container) {
    ...
    window.showStationFullReport = () => {
        ...
        let html = `<h5>${station.name} (${station.code})</h5><table class="table table-bordered"><thead>运转<th>الأصل</th><th>الحالة</th><th>سبب التوقف</th><th>المصدر</th> </thead><tbody>`;
    ...
    window.printStationReport = () => {
        ...
        const win = window.open('', '_blank');
        win.document.write(`<html dir="rtl"><head><title>تقرير متابعة ${stationName}</title><style>body{font-family:Tahoma;padding:20px} table{border-collapse:collapse;width:100%} th,td{border:1px solid #ddd;padding:8px}</style></head><body><h2>تقرير متابعة محطة ${stationName}</h2>${content}</body></html>`);
        win.document.close();
        win.print();
    };
}
...
async function loadReportAsset(container) {
    ...
    window.showAssetFullReport = () => {
        ...
        let html = `<h5>تقرير متابعة الأصل: ${assetName}</h5><p><strong>المحطة:</strong> ${station?.name} (${station?.code})</p>
                    <div class="table-responsive"><table class="table table-bordered"><thead>运转<th>المصدر</th><th>الحالة</th><th>سبب التوقف</th><th>ملاحظات / تفاصيل</th> </thead><tbody>
                    <tr><td>آخر تقرير متابعة دورية<br><small>${lastDate}</small></td><td><span class="badge ${assetStatus === 'يعمل' ? 'bg-success' : (assetStatus === 'لا يعمل' ? 'bg-danger' : 'bg-secondary')}">${assetStatus || '-'}</span></td><td>${assetReason || '-'}</td><td>${assetNotes || '-'}</td></tr>`;
        if (assetFaults.length) {
            assetFaults.forEach(f => {
                html += `<tr style="background-color:#fff3cd"><td>بلاغ عطل مفتوح<br><small>${f.date}</small></td><td><span class="badge bg-danger">لا يعمل</span></td><td>${f.description || '-'}</td><td><strong>الإجراء:</strong> ${f.actions || '-'}<br><strong>قطع الغيار:</strong> ${f.parts || '-'}</td></tr>`;
            });
        } else {
    ...
    window.printAssetReport = () => {
        ...
        const win = window.open('', '_blank');
        win.document.write(`<html dir="rtl"><head><title>تقرير متابعة أصل ${assetName}</title><style>body{font-family:Tahoma;padding:20px} table{border-collapse:collapse;width:100%} th,td{border:1px solid #ddd;padding:8px}</style></head><body><h2>تقرير متابعة أصل: ${assetName}</h2>${content}</body></html>`);
        win.document.close();
        win.print();
    };
}
...
async function loadReportEmployees(container) {
    ...
    window.showEmployeesReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>الكود</th><th>الاسم</th><th>المحطة</th><th>الوظيفة</th><th>الوردية</th><th>التليفون</th><th>الحالة</th> </thead><tbody>`;
        filtered.forEach(e => {
...
async function loadReportSpareParts(container) {
    ...
    window.showSparePartsReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>التاريخ</th><th>المحطة</th><th>الأصل المعطل</th><th>قطع الغيار المستخدمة</th><th>نوع العطل</th> </thead><tbody>`;
        filtered.forEach(f => {
            const station = stations.find(s => s.id == f.stationId);
...
async function loadReportWaterPumped(container) {
    ...
    window.showWaterPumpedReport = () => {
        ...
        let totalWater = 0;
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>الشهر</th><th>المحطة</th><th>ساعات التشغيل</th><th>التصرف (m³/h)</th><th>المياه المرفوعة (m³)</th> </thead><tbody>`;
        filtered.forEach(c => {
...
async function loadReportElectricity(container) {
    ...
    window.showElectricityReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>الشهر</th><th>المحطة</th><th>المحول</th><th>الاستهلاك (kWh)</th><th>التكلفة (جنيه)</th> </thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportWaterConsumption(container) {
    ...
    window.showWaterConsumptionReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>الشهر</th><th>المحطة</th><th>الاستهلاك (m³)</th><th>التكلفة (جنيه)</th> </thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportDiesel(container) {
    ...
    window.showDieselReport = () => {
        ...
        if (filtered.length === 0) { resultDiv.innerHTML = '<div class="alert alert-info">لا توجد بيانات</div>'; return; }
        let totalConsumed = 0;
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>الشهر</th><th>المحطة</th><th>المولد</th><th>الاستهلاك (لتر)</th><th>الرصيد المتبقي (لتر)</th> </thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportSparePartsAdvanced(container) {
    ...
    window.showSparePartsAdvancedReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead>运转<th>التاريخ</th><th>المحطة</th><th>الأصل المعطل</th><th>قطع الغيار المستخدمة</th><th>نوع العطل</th><th>إجراءات الإصلاح</th> </thead><tbody>`;
        filtered.forEach(f => {
            const station = stations.find(s => s.id == f.stationId);
...
// ==================== دوال الأرشيف وإدارة المستخدمين ====================
...
async function loadUsers(container) {
    ...
    container.innerHTML = `
        ...
        <div class="table-responsive mt-4">
            <table class="table table-bordered">
                <thead>
                    运转<th>الكود</th><th>الاسم</th><th>الصلاحية</th><th>تاريخ الإضافة</th><th></th> </thead>
