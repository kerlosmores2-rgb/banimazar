...
// ==================== باقي التقارير (مختصرة للاختصار لكن بنفس المنطق) ====================
...
async function loadReportAsset(container) {
    ...
    window.showAssetFullReport = () => {
        ...
        let html = `<h5>تقرير متابعة الأصل: ${assetName}</h5><p><strong>المحطة:</strong> ${station?.name} (${station?.code})</p>
                    <div class="table-responsive"><table class="table table-bordered"><thead><tr><th>المصدر</th><th>الحالة</th><th>سبب التوقف</th><th>ملاحظات / تفاصيل</th></tr></thead><tbody>
...
async function loadReportEmployees(container) {
    ...
    window.showEmployeesReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>الكود</th><th>الاسم</th><th>المحطة</th><th>الوظيفة</th><th>الوردية</th><th>التليفون</th><th>الحالة</th></tr></thead><tbody>`;
        filtered.forEach(e => {
...
async function loadReportSpareParts(container) {
    ...
    window.showSparePartsReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>التاريخ</th><th>المحطة</th><th>الأصل المعطل</th><th>قطع الغيار المستخدمة</th><th>نوع العطل</th></tr></thead><tbody>`;
        filtered.forEach(f => {
            const station = stations.find(s => s.id == f.stationId);
...
async function loadReportWaterPumped(container) {
    ...
    window.showWaterPumpedReport = () => {
        ...
        let totalWater = 0;
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>الشهر</th><th>المحطة</th><th>ساعات التشغيل</th><th>التصرف (m³/h)</th><th>المياه المرفوعة (m³)</th></tr></thead><tbody>`;
        filtered.forEach(c => {
...
async function loadReportElectricity(container) {
    ...
    window.showElectricityReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>الشهر</th><th>المحطة</th><th>المحول</th><th>الاستهلاك (kWh)</th><th>التكلفة (جنيه)</th></tr></thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportWaterConsumption(container) {
    ...
    window.showWaterConsumptionReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>الشهر</th><th>المحطة</th><th>الاستهلاك (m³)</th><th>التكلفة (جنيه)</th></tr></thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportDiesel(container) {
    ...
    window.showDieselReport = () => {
        ...
        if (filtered.length === 0) { resultDiv.innerHTML = '<div class="alert alert-info">لا توجد بيانات</div>'; return; }
        let totalConsumed = 0;
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>الشهر</th><th>المحطة</th><th>المولد</th><th>الاستهلاك (لتر)</th><th>الرصيد المتبقي (لتر)</th></tr></thead><tbody>`;
        filtered.forEach(c => {
            const station = stations.find(s => s.id == c.stationId);
...
async function loadReportSparePartsAdvanced(container) {
    ...
    window.showSparePartsAdvancedReport = () => {
        ...
        let html = `<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>التاريخ</th><th>المحطة</th><th>الأصل المعطل</th><th>قطع الغيار المستخدمة</th><th>نوع العطل</th><th>إجراءات الإصلاح</th></tr></thead><tbody>`;
        filtered.forEach(f => {
            const station = stations.find(s => s.id == f.stationId);
