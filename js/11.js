...
function loadHome(container) {
    ...
    container.innerHTML = `
        ...
        <div class="form-card">
            <h4>مرحباً بك</h4>
            <p>نظام إدارة محطات رفع الصرف الصحي ببني مزار</p>
            ...
            <form id="stationForm">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="stationCode" class="form-control" required>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        <label>المصب</label>
                        <input type="text" id="outfall" class="form-control">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label>القدرة التصميمية (m³/h)</label>
        ...
        <div class="form-card">
            ...
            <form id="empForm">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label>المحطة</label>
                        <input type="text" id="empStation" class="form-control">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>الوظيفة</label>
...
function loadAddFault(container) {
    container.innerHTML = `<div class="form-card"><h4>⚠️ بلاغ عطل</h4><p>جاري التطوير</p></div>`;
}

function loadListFaults(container) {
    container.innerHTML = `<div class="form-card"><h4>📋 قائمة البلاغات</h4><p>جاري التطوير</p></div>`;
}

function loadTariffs(container) {
    container.innerHTML = `<div class="form-card"><h4>💰 تعريفات</h4><p>جاري التطوير</p></div>`;
}

function loadMonthlyCosts(container) {
    container.innerHTML = `<div class="form-card"><h4>📊 تقارير شهرية</h4><p>جاري التطوير</p></div>`;
}

function loadReportDaily(container) {
    container.innerHTML = `<div class="form-card"><h4>📝 متابعة دورية</h4><p>جاري التطوير</p></div>`;
}

function loadReportFaults(container) {
    container.innerHTML = `<div class="form-card"><h4>🐛 تقرير الأعطال</h4><p>جاري التطوير</p></div>`;
}

function loadReportStation(container) {
    container.innerHTML = `<div class="form-card"><h4>🏭 متابعة محطة</h4><p>جاري التطوير</p></div>`;
}

function loadReportAsset(container) {
    container.innerHTML = `<div class="form-card"><h4>⚙️ متابعة أصل</h4><p>جاري التطوير</p></div>`;
}

function loadReportEmployees(container) {
    container.innerHTML = `<div class="form-card"><h4>👥 تقرير العاملين</h4><p>جاري التطوير</p></div>`;
}

function loadReportSpareParts(container) {
    container.innerHTML = `<div class="form-card"><h4>🔧 تقرير قطع غيار</h4><p>جاري التطوير</p></div>`;
}

function loadArchive(container) {
    container.innerHTML = `<div class="form-card"><h4>📁 الأرشيف</h4><p>جاري التطوير</p></div>`;
}

function loadUsers(container) {
    container.innerHTML = `<div class="form-card"><h4>⚙️ إدارة المستخدمين</h4><p>جاري التطوير</p></div>`;
}

// ==================== الدالة الرئيسية لتحميل الصفحات ====================
function loadPage(page) {
    // التأكد من الجلسة أولاً
