...
async function loadHome(container) {
    ...
    const { data: employees, error: employeesErr } = await supabase.from('employees').select('*');
    const { data: faults, error: faultsErr } = await supabase.from('faults').select('*');
    ...
    container.innerHTML = `
        <div class="row">
            <div class="col-md-4"><div class="stats-card"><i class="fas fa-water"></i><h3>${stationsCount}</h3><p>عدد المحطات</p></div></div>
            <div class="col-md-4"><div class="stats-card"><i class="fas fa-users"></i><h3>${employeesCount}</h3><p>عدد العاملين</p></div></div>
...
async function loadAddStation(container) {
    ...
    document.getElementById('stationForm').onsubmit = async function(e) {
        ...
        const { data, error } = await supabase.from('stations').insert([newStation]).select();
        
        if (error) {
            console.error('خطأ:', error);
            alert('فشل في حفظ المحطة: ' + error.message);
        } else {
...
async function loadListStations(container) {
    ...
    if (error) {
        console.error('خطأ:', error);
        container.innerHTML = '<div class="alert alert-danger">خطأ في تحميل المحطات</div>';
        return;
    }
    ...
    let html = `<div class="search-bar"><input type="text" id="searchInput" class="form-control" placeholder="بحث..."></div>
                ...
                <tr><th>الكود</th><th>الاسم</th><th>النوع</th><th>المصب</th><th></th></tr>
                </thead><tbody id="stationsTable">`;
    ...
    stations.forEach(s => {
        html += `<tr>
                    ...
                    <td>${s.outfall || '-'}</td>
                    <td>
                        <button class="btn btn-sm btn-warning" onclick="editStation(${s.id})">?? تعديل</button>
                        <button class="btn btn-sm btn-danger" onclick="deleteStationNow(${s.id})">??? حذف</button>
    ...
    window.viewStation = (id) => {
        ...
        container.innerHTML = `<div class="form-card"><h4>${station.name} (${station.code})</h4>
...
async function loadEditStation(container, stationId) {
    ...
    let outfallOptions = '<option value="">بدون</option>';
    stations.forEach(s => {
        outfallOptions += `<option value="${s.name}" ${station.outfall == s.name ? 'selected' : ''}>${s.name}</option>`;
    });
    
    container.innerHTML = `
        <div class="form-card">
            <h4>?? تعديل المحطة: ${station.name}</h4>
    ...
    document.getElementById('editStationForm').onsubmit = async function(e) {
        ...
        const { error } = await supabase.from('stations').update(updatedStation).eq('id', stationId);
...
async function loadListEmployees(container) {
    const { data: employees, error: empError } = await supabase.from('employees').select('*').order('name');
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    
    if (empError || staError) {
    ...
    let html = `<div class="search-bar"><input type="text" id="searchEmpInput" class="form-control" placeholder="بحث..."></div>
                <div class="table-responsive"><table class="table table-bordered"><thead>
                <tr><th>الكود</th><th>الاسم</th><th>المحطة</th><th>الوظيفة</th><th>الوردية</th><th>التليفون</th><th>الحالة</th><th></th></tr>
                </thead><tbody id="empTable">`;
    ...
    employees.forEach(e => {
        html += `<tr>
                    ...
                    <td>${e.status}</td>
                    <td>
                        <button class="btn btn-sm btn-warning" onclick="editEmployee(${e.id})">?? تعديل</button>
                        <button class="btn btn-sm btn-danger" onclick="deleteEmployeeNow(${e.id})">??? حذف</button>
                    </td>
                </tr>`;
    });
...
async function loadEditEmployee(container, employeeId) {
    ...
    let stationOptions = '<option value="">اختر محطة</option>';
    stations.forEach(s => {
        stationOptions += `<option value="${s.id}" ${employee.stationId == s.id ? 'selected' : ''}>${s.name}</option>`;
    });
    
    container.innerHTML = `
        <div class="form-card">
            <h4>?? تعديل بيانات العامل</h4>
            <form id="editEmpForm">
                <div class="row">
    ...
    document.getElementById('editEmpForm').onsubmit = async function(e) {
        ...
        const { error } = await supabase.from('employees').update(updatedEmployee).eq('id', employeeId);
...
async function loadAddFault(container) {
    ...
    document.getElementById('faultStation').addEventListener('change', async function() {
        ...
        const { data: station, error } = await supabase.from('stations').select('*').eq('id', stationId).single();
        if (station) {
...
async function loadListFaults(container) {
    const { data: faults, error: faultError } = await supabase.from('faults').select('*').order('date', { ascending: false });
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    
    if (faultError || staError) {
        container.innerHTML = '<div class="alert alert-danger">خطأ في تحميل البيانات</div>';
        return;
    }
    ...
    let stationFilter = '<option value="">كل المحطات</option>';
    if (stations) {
        stationFilter += stations.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    }
    ...
    function displayFaultsList(list) {
        ...
        let html = '<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>المحطة</th><th>الأصل</th><th>النوع</th><th>التاريخ</th><th>الحالة</th><th></th></tr></thead><tbody>';
        list.forEach(f => {
            html += `<tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning" onclick="editFault(${f.id})">?? تعديل</button>
    ...
    window.viewFaultDetail = async (id) => {
        const { data: fault, error } = await supabase.from('faults').select('*').eq('id', id).single();
    ...
    window.closeFaultNow = async (id) => {
        const { error } = await supabase.from('faults').update({ status: 'closed' }).eq('id', id);
        if (error) {
            alert('خطأ: ' + error.message);
        } else {
...
async function loadEditFault(container, faultId) {
    const { data: fault, error: faultError } = await supabase.from('faults').select('*').eq('id', faultId).single();
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    
    if (faultError || staError || !fault) {
        alert('خطأ في تحميل البيانات');
        return;
    }
    ...
    let stationOptions = '<option value="">اختر محطة</option>';
    stations.forEach(s => {
        stationOptions += `<option value="${s.id}" ${fault.stationId == s.id ? 'selected' : ''}>${s.name}</option>`;
    });
    ...
    container.innerHTML = `
        <div class="form-card">
            ...
            <form id="editFaultForm">
                ...
                <button type="submit" class="btn btn-primary">?? حفظ التعديل</button>
                <button type="button" class="btn btn-danger" onclick="deleteFaultNow(${fault.id})">??? حذف البلاغ</button>
    ...
    document.getElementById('editFaultForm').onsubmit = async function(e) {
        ...
        const { error } = await supabase.from('faults').update(updatedFault).eq('id', faultId);
...
async function loadTariffs(container) {
    const { data: tariffs, error } = await supabase.from('tariffs').select('*').limit(1);
    
    let electricity = 1.2;
    let water = 3.5;
    
    if (tariffs && tariffs.length > 0) {
        electricity = tariffs[0].electricity;
        water = tariffs[0].water;
    }
    ...
    container.innerHTML = `
        <div class="form-card">
            <h4>?? تعريفات</h4>
            <form id="tariffsForm">
                <div class="mb-3"><label>تعريفة الكهرباء (جنيه/كيلو وات)</label><input type="number" step="0.01" id="elecTariff" class="form-control" value="${electricity}"></div>
                <div class="mb-3"><label>تعريفة المياه (جنيه/متر مكعب)</label><input type="number" step="0.01" id="waterTariff" class="form-control" value="${water}"></div>
                <button type="submit" class="btn btn-primary">حفظ</button>
            </form>
        </div>
    `;
    ...
    document.getElementById('tariffsForm').onsubmit = async function(e) {
        ...
        const { error } = await supabase.from('tariffs').update(newTariffs).eq('id', 1);
        
        if (error && error.code !== 'PGRST116') {
            const { error: insertError } = await supabase.from('tariffs').insert([{ id: 1, ...newTariffs }]);
            if (insertError) {
...
async function loadMonthlyCosts(container) {
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    const { data: tariffs, error: tarError } = await supabase.from('tariffs').select('*').limit(1);
    const { data: monthlyCosts, error: costError } = await supabase.from('monthly_costs').select('*');
    
    let currentTariffs = { electricity: 1.2, water: 3.5 };
    if (tariffs && tariffs.length > 0) {
        currentTariffs = tariffs[0];
    }
            ...
            <form id="monthlyForm">
                ...
                <div class="mt-4 p-3 border rounded">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4"><label>التعريفة</label><input type="text" id="waterTariff" class="form-control" value="${currentTariffs.water}" readonly></div>
    ...
    window.showPreviousReports = () => {
        ...
        if (reports.length === 0) {
            ...
            let html = '<div class="form-card"><h5>التقارير السابقة</h5><table class="table"><thead>??<th>الشهر</th><th>المياه المرفوعة</th><th>كهرباء</th><th>سولار</th><th></th> </thead><tbody>';
            reports.forEach(r => {
                html += `??
                            <td>${r.month} \n                             \n                             \n                             \n
    ...
    document.getElementById('monthStation').addEventListener('change', async function() {
        const stationId = this.value;
        const station = stations?.find(s => s.id == stationId);
        if (station) {
                ...
                if (transformers.length) {
                    ...
                    document.querySelectorAll('.transformer-current').forEach(inp => {
                        inp.addEventListener('input', function() {
                            ...
                            const cost = diff * currentTariffs.electricity;
    ...
    function calculateWater() {
        ...
        const cost = consumed * currentTariffs.water;
    ...
    document.getElementById('monthlyForm').onsubmit = async function(e) {
        ...
        const waterCost = waterConsumed * currentTariffs.water;
        ...
        if (existingReport) {
            if (confirm('يوجد تقرير لهذا الشهر. هل تريد تحديثه؟')) {
                const { error } = await supabase.from('monthly_costs').update(report).eq('id', existingReport.id);
                if (error) alert('خطأ: ' + error.message);
                else alert('تم تحديث التقرير');
            } else return;
        } else {
            const { error } = await supabase.from('monthly_costs').insert([report]);
            if (error) alert('خطأ: ' + error.message);
            else alert('تم حفظ التقرير');
        }
        loadPage('home');
    };
...
async function loadReportDaily(container) {
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    const { data: dailyReports, error: reportError } = await supabase.from('daily_reports').select('*');
    ...
    window.showDailyReportsList = () => {
        ...
        if (reports.length === 0) listDiv.innerHTML = '<div class="alert alert-info">لا توجد تقارير</div>';
            ...
            let html = '<div class="form-card"><h5>قائمة التقارير</h5><table class="table"><thead>??<th>المحطة</th><th>التاريخ</th><th>المتابع</th><th></th> </thead><tbody>';
            reports.forEach(r => {
                html += `??
                            <td>${r.stationName} \n                             \n                             \n
...
// ==================== دوال التقارير المتبقية (نسخ احتياطي) ====================
async function loadReportFaults(container) {
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    const { data: faults, error: faultError } = await supabase.from('faults').select('*');
    
    let stationOptions = '<option value="">كل المحطات</option>';
    if (stations) {
        stationOptions += stations.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    }
    ...
    window.showFaultsReport = () => {
        ...
        filtered.forEach(f => {
            ...
            html += `  <td>${station?.name || f.stationName || '-'}   <td>${f.assetName || '-'}   <td>${f.type}   <td>${f.date}   <td>${f.description}   <td>${f.actions || '-'}   <td>${f.status === 'open' ? 'مفتوح' : 'مغلق'}   `;
        });
        html += '</tbody> </div>';
...
async function loadReportStation(container) {
    const { data: stations, error: staError } = await supabase.from('stations').select('*');
    const { data: dailyReports, error: dailyError } = await supabase.from('daily_reports').select('*');
    const { data: faults, error: faultError } = await supabase.from('faults').select('*');
    ...
    window.showStationFullReport = () => {
        ...
        allAssets.forEach(name => {
            ...
            html += `  <td>${name}   <td>${status}   <td>${reason}   <td>${source}   `;
        });
        html += '</tbody> </div>';
...
async function loadReportAsset(container) {
    ...
    container.innerHTML = `
        <div class="form-card">
            ...
            <button id="printAssetBtn" class="btn btn-secondary mt-3" style="display:none" onclick="printAssetReport()">??? طباعة</button>
        </div>
    `;
        ...
        let html = `<h5 class="mb-3">تقرير متابعة الأصل: ${assetName}</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                 <td>آخر تقرير متابعة دورية<br><small class="text-muted">${lastDate}</small> <td><span class="badge ${assetStatus === 'يعمل' ? 'bg-success' : (assetStatus === 'لا يعمل' ? 'bg-danger' : 'bg-secondary')}">${assetStatus || '-'}</span> <td>${assetReason || '-'} <td>${assetNotes || '-'} `;
        
        if (assetFaults.length) {
            assetFaults.forEach(f => {
                html += `<tr style="background-color:#fff3cd">
                             <td>بلاغ عطل مفتوح<br><small class="text-muted">${f.date}</small> <td><span class="badge bg-danger">لا يعمل</span> <td>${f.description || '-'} <td><strong>الإجراء:</strong> ${f.actions || '-'}<br><strong>قطع الغيار:</strong> ${f.parts || '-'}
                          </tr>`;
            });
        } else {
            html += `<tr><td>بلاغات الأعطال<td colspan="3">لا توجد بلاغات مفتوحة لهذا الأصل</tr>`;
        }
